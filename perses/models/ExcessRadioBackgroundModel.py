...
"""
File: perses/models/ExcessRadioBackgroundModel.py
Author: David W. Barker
Date: 24 Mar 2026

Description: File containing class extending pylinex's Model class to model
             global 21-cm signals using the excess radio background formulation, but only
...
class ExcessRadioBackgroundModel(LoadableModel):
    """Class extending pylinex's Model class to model global 21-cm signals using
    the the math behind adiabatic expansion, compton scattering, stimulated emission, and modifications from excess radio background sources
    but keep in mind that this does not work beyond the Dark Ages as of now."""
    
    def __init__(self, frequencies, in_Kelvin=False):
        """
        Initializes a new ExcessRadioBackgroundModel applying to the given frequencies.
    ...
    def __call__(self, parameters):
        """
        Evaluates this ExcessRadioBackgroundModel at the given parameter values.
        
        parameters: array of length 2, with the first value being the turnover redshift and the second being the amplitude of the excess radio background.

        returns: dark matter baryon interaction model evaluated at the given parameters
        """
        if len(parameters) != 2:
            raise ValueError("There should be 2 parameters given to the ExcessRadioBackgroundModel: the turnover redshift: z_s,\
                            , and the amplitude of the excess radio background: A_r" )
        try:
            model_parameters = [[parameters[0],parameters[1]]]
            
            raw_values = np.array(py21cmsig.ERB_training_set(np.arange(2,50,0.5),model_parameters,N=1,verbose=False)[0][0])
    ...
    def fill_hdf5_group(self, group):
        ...
        """
        group.attrs['class'] = 'ExcessRadioBackgroundModel'
        group.attrs['import_string'] =\
            'from perses.models import ExcessRadioBackgroundModel'
    ...
    def load_from_hdf5_group(group):
        ...
        return ExcessRadioBackgroundModel(frequencies, in_Kelvin=in_Kelvin)
    ...
    def __eq__(self, other):
        ...
        """
        if not isinstance(other, ExcessRadioBackgroundModel):
            return False
