...
"""
File: perses/models/PrimordialBlackHolesModel.py
Author: David W. Barker
Date: 24 Mar 2026

Description: File containing class extending pylinex's Model class to model
             global 21-cm signals using the primordial black holes formulation, but only
...
class PrimordialBlackHolesModel(LoadableModel):
    """Class extending pylinex's Model class to model global 21-cm signals using
    the the math behind adiabatic expansion, compton scattering, stimulated emission, and heating from primordial black holes 
    but keep in mind that this does not work beyond the Dark Ages as of now."""
    
    def __init__(self, frequencies, in_Kelvin=False):
        """
        Initializes a new PrimordialBlackHolesModel applying to the given frequencies.
    ...
    def __call__(self, parameters):
        """
        Evaluates this PrimordialBlackHolesModel at the given parameter values.
        
        parameters: array of length 2, with the first value being the root mean square mass of primordial black holes: m_bh
                    ,and the second being the density parameter of black holes as dark matter: ombh 

        returns: primordial black holes model evaluated at the given parameters
        """
        if len(parameters) != 2:
            raise ValueError("There should be 2 parameters given to the PrimordialBlackHolesModel: root mean square mass of primordial black holes: m_bh,\
                            , density parameter of black holes as dark matter: ombh" )
        try:
            model_parameters = [[parameters[0],parameters[1]]]
            
            raw_values = np.array(py21cmsig.PBH_training_set(np.arange(1,50,0.5),model_parameters,N=1,verbose=False)[0][0])
    ...
    def fill_hdf5_group(self, group):
        ...
        """
        group.attrs['class'] = 'PrimordialBlackHolesModel'
        group.attrs['import_string'] =\
            'from perses.models import PrimordialBlackHolesModel'
    ...
    def load_from_hdf5_group(group):
        ...
        return PrimordialBlackHolesModel(frequencies, in_Kelvin=in_Kelvin)
    ...
    def __eq__(self, other):
        ...
        """
        if not isinstance(other, PrimordialBlackHolesModel):
            return False
