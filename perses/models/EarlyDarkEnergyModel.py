...
"""
File: perses/models/EarlyDarkEnergyModel.py
...
class EarlyDarkEnergyModel(LoadableModel):
    ...
    def __init__(self, frequencies, in_Kelvin=False):
        """
        Initializes a new EarlyDarkEnergyModel applying to the given frequencies.
    ...
    def __call__(self, parameters):
        """
        Evaluates this EarlyDarkEnergyModel at the given parameter values.
        ...
        """
        if len(parameters) != 2:
            raise ValueError("There should be 2 parameters given to the EarlyDarkEnergyModel: the density parameter for early dark energy: Omega_ee,\
, and the turnover redshift: z_c" )
        try:
            model_parameters = parameters
            
            raw_values = np.array(py21cmsig.EDE_training_set(np.arange(5,50,0.5),model_parameters,N=1,verbose=False)[0][0])
            interpolator = scipy.interpolate.CubicSpline(np.arange(5,50,0.5),raw_values)
            signal_in_mK = interpolator(self.frequencies)
        except ValueError:
            print("The Early Dark Energy Model evaluated an infinite number based on a parameter input. This value will be ignored") 
    ...
    def fill_hdf5_group(self, group):
        ...
        """
        group.attrs['class'] = 'EarlyDarkEnergyModel'
        group.attrs['import_string'] =\
            'from perses.models import EarlyDarkEnergyModel'
    ...
    def load_from_hdf5_group(group):
        ...
        return EarlyDarkEnergyModel(frequencies, in_Kelvin=in_Kelvin)
    ...
    def __eq__(self, other):
        ...
        """
        if not isinstance(other, EarlyDarkEnergyModel):
            return False
